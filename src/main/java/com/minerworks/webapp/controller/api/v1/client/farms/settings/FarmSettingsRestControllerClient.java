package com.minerworks.webapp.controller.api.v1.client.farms.settings;
...
public class FarmSettingsRestControllerClient {

    private Logger logger = LoggerFactory.getLogger(FarmSettingsRestControllerClient.class);
    ...
    public ResponseEntity<FarmSettingsEntity> deleteFarmSettingsEntity(Principal principal, @PathVariable("farmUUID") String farmCUUID) {
        String email = principal.getName();
        User user = userRepository.findByEmail(email);

        logger.debug("Deleting FarmSettingsEntity for user {} and farm (farm uuid) {}", user, farmCUUID);

        FarmSettingsEntity farmSettingsEntity = farmEntityService.getFarmSettingsEntity(user, farmCUUID);

        if (farmSettingsEntity == null) {
            logger.debug("Unable to delete! FarmSettingsEntity for user {} and farm (farm uuid) {} not found!", user, farmCUUID);
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }

        farmEntityService.deleteFarmSettingsEntity(user, farmCUUID);
