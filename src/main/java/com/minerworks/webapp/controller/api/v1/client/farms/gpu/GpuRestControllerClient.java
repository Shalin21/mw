package com.minerworks.webapp.controller.api.v1.client.farms.gpu;
...
public class GpuRestControllerClient {

    private Logger logger = LoggerFactory.getLogger(GpuRestControllerClient.class);
    ...
    /*
    all clients have rights but intended for frontend and mobile clients
     */
    @GetMapping(value = "/{farmUUID}/gpu")
    @ResponseBody
    public ResponseEntity<Set<GpuCardEntity>> getFarmGpus(Principal principal, @PathVariable("farmUUID") String farmUUID) {
        User user = userRepository.findByEmail(principal.getName());
        logger.debug("Get gpus for farm. \n User: {} \n Farm (uuid): {} \n", user, farmUUID);
        return ResponseEntity.ok(farmEntityService.getGpuCardEntitySet(user, farmUUID));
    }
